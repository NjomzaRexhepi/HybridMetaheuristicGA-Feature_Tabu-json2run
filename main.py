...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_file", type=str, default="B60_L6_D6.txt", help="Input File")
    parser.add_argument("--max_evaluations", type=int, default=20,
                        help="Maximum number of hyperparameter configurations to try.")
    parser.add_argument("--neps_root_dir", type=str, default="neps_results_B60_L6_D6_max",
    ...
    pipeline_space = dict(
        population_size=neps.IntegerParameter(lower=100, upper=500, log=False),  
...
# #     # NePS only minimizes, so we return -fitness_score is we want to maximaze or minimaze
# #     return {
# #         "loss": -fitness_score, # qetu kom ndryshi prej fitness_score ne negativ
# #         # maybe i should change this to fitness_score  , changed from  -fitness_score -> fitness_score
# #         "cost": runtime,
# #         # Store extra information
# #         "info_dict": {
# #             "fitness_score": fitness_score,
...
# #     parser.add_argument("--max_evaluations", type=int, default=20,
# #                         help="Maximum number of hyperparameter configurations to try.")
# #     parser.add_argument("--neps_root_dir", type=str, default="neps_results_B60_L6_D6_forth_run",
# #                         help="Directory to store NePS results.")
...
# #     # # better for smaller files
# #     pipeline_space = dict(
# #         population_size=neps.IntegerParameter(lower=3, upper=2000, log=False),  # Lower population size
# #         mutation_prob=neps.FloatParameter(lower=0.01, upper=0.8, log=False),  # Higher mutation for more exploration
# #         hill_climbing_steps=neps.IntegerParameter(lower=0, upper=1000, log=False),
# #         # Fewer steps to avoid long refinements
# #         tabu_length=neps.IntegerParameter(lower=0, upper=50, log=False)  # Smaller tabu memory to allow revisiting
# #     )
...
# #         for config_id, config_data in all_configs_and_losses.items():
# #             if config_data.result is None:
# #                 continue
# #             if 'loss' in config_data.result and config_data.result['loss'] < best_loss:
# #                 best_loss = config_data.result['loss']
...
# #             print("\n--- Best Configuration Found by NePS ---")
# #             print(f"Config ID: {best_config_details['config_id']}")
# #             print(f"Best Loss (Minimized -Fitness): {best_config_details['loss']:.4f}")
# #             if 'fitness_score' in best_config_details['info']:
# #                 print(f"Corresponding Fitness Score: {best_config_details['info']['fitness_score']:.4f}")
# #             print("Hyperparameters:")
# #             for key, value in best_config_details['config'].items():
